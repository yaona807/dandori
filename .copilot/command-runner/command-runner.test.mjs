...
async function makeFixture(configure = (configuration) => configuration) {
  ...
  await writeFile(path.join(home, 'command-runner', 'command-runner.mjs'), await readFile(RUNNER_SOURCE));
  await writeFile(path.join(home, 'command-runner', 'command-runner-hook.mjs'), await readFile(HOOK_SOURCE));
  await writeFile(path.join(home, 'agents', 'CommandRunner.agent.md'), await readFile(AGENT_SOURCE));
  await writeFile(path.join(alpha, 'echo-args.mjs'), 'process.stdout.write(JSON.stringify(process.argv.slice(2)));\n');
  await writeFile(path.join(beta, 'echo-args.mjs'), 'process.stdout.write(JSON.stringify(["beta", ...process.argv.slice(2)]));\n');
  await writeFile(path.join(alpha, 'tests', 'sample.test.js'), 'export {};\n');
        ...
        commands: {
          sample: {
            ...
            arguments: {
              ...
              count: {
                kind: 'option', token: '--count',
                value: { type: 'integer', min: 1, max: 8 },
              },
              mode: {
                kind: 'option', token: '--mode',
                value: { type: 'choice', values: ['fast', 'safe'] },
              },
              file: {
                kind: 'positional',
                value: { type: 'workspace-file', extensions: ['.test.js'], mustExist: true },
              },
              text: {
                kind: 'positional',
...
function runRunner(fixture, cwd, args) {
  return spawnSync(process.execPath, [path.join(fixture.home, 'command-runner', 'command-runner.mjs'), ...args], {
    cwd,
    encoding: 'utf8',
...
function runHook(fixture, input) {
  return spawnSync(process.execPath, [path.join(fixture.home, 'command-runner', 'command-runner-hook.mjs')], {
    cwd: fixture.alpha,
    encoding: 'utf8',
...
async function withFixture(callback, configure) {
  ...
  try { await callback(fixture); } finally { await rm(fixture.root, { recursive: true, force: true }); }
}
...
test('list exposes commands only for the current workspace', async () => {
  await withFixture(async (fixture) => {
    ...
    assert.deepEqual(alphaOutput.commands.map(({ id }) => id), ['sample']);
...
test('deepest registered root wins for nested workspaces', async () => {
  await withFixture(async (fixture) => {
    ...
    await mkdir(nested);
    await writeFile(path.join(nested, 'echo-args.mjs'), 'process.stdout.write("nested");\n');
    const configPath = path.join(fixture.home, 'command-runner', 'workspaces.json');
    const config = JSON.parse(await readFile(configPath, 'utf8'));
    config.workspaces.push({
      id: 'nested', root: nested,
      commands: { nested: { description: 'Nested command.', run: [process.execPath, 'echo-args.mjs'], cwd: '.', arguments: {} } },
    });
...
test('run builds deterministic argv from typed named arguments', async () => {
  await withFixture(async (fixture) => {
    const result = runRunner(fixture, fixture.alpha, [
      'run', 'sample', 'text=hello%20world', 'file=tests%2Fsample.test.js',
      'mode=safe', 'count=4', 'enabled=true',
    ]);
    assert.equal(result.status, 0, result.stderr);
    ...
    assert.deepEqual(JSON.parse(output.stdout), [
      '--', '--enabled', '--count', '4', '--mode', 'safe', 'tests/sample.test.js', 'hello world',
    ]);
  });
});
...
test('shell-looking string stays one argv value', async () => {
  await withFixture(async (fixture) => {
    const result = runRunner(fixture, fixture.alpha, ['run', 'sample', 'text=%24%28touch%20owned%29']);
    assert.equal(result.status, 0, result.stderr);
    assert.deepEqual(JSON.parse(JSON.parse(result.stdout).stdout), ['--', '$(touch owned)']);
  });
});

test('unknown arguments and out-of-range values are denied', async () => {
  await withFixture(async (fixture) => {
    const unknown = runRunner(fixture, fixture.alpha, ['run', 'sample', 'other=value']);
    assert.equal(unknown.status, 2);
    assert.match(unknown.stderr, /unregistered parameter/u);
    const range = runRunner(fixture, fixture.alpha, ['run', 'sample', 'count=99']);
...
test('workspace path traversal and symlink escapes are denied', async (t) => {
  await withFixture(async (fixture) => {
    const traversal = runRunner(fixture, fixture.alpha, ['run', 'sample', 'file=..%2Foutside.test.js']);
    ...
    try { await symlink(target, alias); } catch (error) {
      if (error?.code === 'EPERM' || error?.code === 'EACCES') { t.skip('symlink creation unavailable'); return; }
      throw error;
    }
    const disguised = runRunner(fixture, fixture.alpha, ['run', 'sample', 'file=tests%2Falias.test.js']);
    ...
    await symlink(outside, path.join(fixture.alpha, 'tests', 'linked.test.js'));
    const escaped = runRunner(fixture, fixture.alpha, ['run', 'sample', 'file=tests%2Flinked.test.js']);
...
test('duplicate configuration keys fail closed', async () => {
  await withFixture(async (fixture) => {
    await writeFile(path.join(fixture.home, 'command-runner', 'workspaces.json'), '{"version":1,"version":1,"workspaces":[]}');
...
test('dynamic arguments cannot be attached to known inline-code forms', async () => {
  await withFixture(async (fixture) => {
    ...
    configuration.workspaces[0].commands.sample.run = ['node', '-e', 'console.log(process.argv[1])'];
    return configuration;
  });
});

test('hook permits canonical user-level runner calls and denies raw commands', async () => {
  await withFixture(async (fixture) => {
    const allowed = runHook(fixture, {
      hook_event_name: 'PreToolUse', tool_name: 'execute/runInTerminal',
      tool_input: { command: 'node ~/.copilot/command-runner/command-runner.mjs run sample count=4' },
    });
    assert.equal(allowed.status, 0, allowed.stderr);
    assert.equal(JSON.parse(allowed.stdout).hookSpecificOutput.permissionDecision, 'allow');

    const raw = runHook(fixture, {
      hook_event_name: 'PreToolUse', tool_name: 'execute/runInTerminal', tool_input: { command: 'npm test' },
    });
    assert.equal(JSON.parse(raw.stdout).hookSpecificOutput.permissionDecision, 'deny');

    const compound = runHook(fixture, {
      hook_event_name: 'PreToolUse', tool_name: 'execute/runInTerminal',
      tool_input: { command: 'node ~/.copilot/command-runner/command-runner.mjs list && npm publish' },
    });
    assert.equal(JSON.parse(compound.stdout).hookSpecificOutput.permissionDecision, 'deny');
  });
});

test('hook ignores non-terminal tools except writes to user-level control files', async () => {
  await withFixture(async (fixture) => {
    const read = runHook(fixture, {
      hook_event_name: 'PreToolUse', tool_name: 'read/readFile', tool_input: { path: 'README.md' },
    });
    assert.deepEqual(JSON.parse(read.stdout), { continue: true });

    const protectedWrite = runHook(fixture, {
      hook_event_name: 'PreToolUse', tool_name: 'edit/editFiles',
      tool_input: { files: [{ path: '~/.copilot/command-runner/workspaces.json', replacement: '{}' }] },
    });
    assert.equal(JSON.parse(protectedWrite.stdout).hookSpecificOutput.permissionDecision, 'deny');

    const unrelated = runHook(fixture, {
      hook_event_name: 'PreToolUse', tool_name: 'edit/editFiles',
      tool_input: { files: [{ path: 'src/example.js', replacement: 'export {};' }] },
    });
