...
const TERMINAL_TOOL_NAMES = new Set(['execute/runInTerminal', 'runInTerminal', 'runTerminalCommand']);
...
function allow(reason) {
  output({ hookSpecificOutput: { hookEventName: 'PreToolUse', permissionDecision: 'allow', permissionDecisionReason: reason } });
}
...
function parseCommand(command) {
  ...
  if (FORBIDDEN_CHARACTER_PATTERN.test(command)) throw new Error('terminal command contains unsupported shell syntax or control characters');
  if (command !== command.trim() || /\s{2,}/u.test(command)) throw new Error('terminal command must use canonical single-space formatting');
  return command.split(' ');
}

function validateEncodedParameter(token) {
  if (!PARAMETER_TOKEN_PATTERN.test(token)) throw new Error(`invalid named argument token: ${token}`);
...
function validateRunnerInvocation(tokens) {
  if (tokens.length < FIXED_PREFIX.length + 1 || !FIXED_PREFIX.every((token, index) => tokens[index] === token)) {
  ...
  if (!COMMAND_ID_PATTERN.test(commandId ?? '')) throw new Error('describe and run require a safe command ID');
...
function normalizePossiblePath(value) {
  ...
  if (normalized.startsWith('file://')) {
    try { normalized = decodeURIComponent(new URL(normalized).pathname); } catch { return null; }
  }
...
function containsProtectedPath(value) {
  if (typeof value === 'string') {
    ...
    return [...PROTECTED_SUFFIXES].some((suffix) => normalized === suffix || normalized.endsWith(`/${suffix}`));
  }
...
async function readInput() {
  ...
  if (!source || source.length > 1_048_576) throw new Error('invalid hook input size');
...
try {
  ...
  if (!TERMINAL_TOOL_NAMES.has(toolName)) {
    if (typeof toolName === 'string' && WRITE_TOOL_NAME_PATTERN.test(toolName) && containsProtectedPath(input?.tool_input)) {
      deny('User-level CommandRunner control files cannot be modified by agent write tools.');
    } else {
      output({ continue: true });
    }
  } else {
    validateRunnerInvocation(parseCommand(input?.tool_input?.command));
    allow('Registered commands are selected from the current workspace by the fixed user-level runner.');
  }
} catch (error) {
