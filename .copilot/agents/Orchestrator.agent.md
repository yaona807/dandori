...
Durable resume uses a storage-agnostic append-only journal. Each event has stable `flow_id`, unique `event_id`, increasing `seq`, `type`, and payload. Types cover authorization/revision, delegation/audit, candidate/verification/criterion transitions, interruption, and terminal state. Events never grant permission; only replayed authorization sources do. Snapshots are optional caches and must match the journal prefix.

Resume by replaying from the first event to rebuild the contract and authorization/loop state, then reconcile `pending_invocation`. Accept a late result only when Task Card/revision matches the replayed pending invocation and active revision; otherwise stale. Missing, duplicate, reordered, truncated, corrupted, or inconsistent replay must stop with `state_unrecoverable`; never guess/reset. Re-observable facts may be reacquired only inside approved observation boundaries.
