...
Durable resume uses a storage-agnostic append-only journal when persistence is available. Each event has stable `flow_id`, unique `event_id`, strictly increasing `seq`, `type`, and compact payload; types cover authorization source/revision, Task Card delegation, result audit/rejection, candidate decision, verification, criterion transition, pending/interruption, and terminal state. Events record history only: permission still comes solely from replayed authorization sources. Snapshots are optional caches and valid only when they match the journal prefix.

On resume, replay from the first event to reconstruct the active contract and all authorization/loop-control state, then reconcile `pending_invocation`. Accept a late result only when its Task Card/revision matches the replayed pending invocation and active revision; otherwise it is stale. Missing, duplicate, reordered, truncated, corrupted, or inconsistent events/replay stop `state_unrecoverable`; never guess/reset. Re-observable facts may be reacquired inside approved observation boundaries; missing permission state may not.
