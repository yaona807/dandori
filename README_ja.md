...
DANDORIは、低い固定実行回数ではなく、現在状態に対する検証済みの改善で反復を制御します。`execute`を含むTask Cardは少なくとも1件の有効なCriterionを参照しなければならず、契約全体の競合またはblockerでCriterionを省略できるのは観察だけの場合に限ります。修正後の検証で、既知の具体的gapの解消またはCriterion/Postconditionの前進が確認され、以前満たしていたCriterionにregressionがない限り、Correction→Verificationは継続できます。新しいEvidence、診断、Worker、Task Card ID、順序、グルーピング、文言だけでは進捗とみなしません。2回連続で実際の進捗が確認できなければCorrectionを停止します。同じ検証コマンドでもmaterial stateが変化した後なら再実行できますが、同一stateへの等価実行を単なる再試行として繰り返すことは禁止し、nondeterminismまたは具体的な競合解消に必要な狭い再実行だけ1回許容します。
