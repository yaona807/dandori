...
ユーザーが承認した作業契約を、最小限の境界付き Task Card へ変換します。Orchestrator は、承認済みの目的・境界・効果・検証水準を変えない限り、内部の実行計画を適応的に変更できます。一方、各 Worker の実行は、毎回発行される狭い Task Card の対象・Action・Effectを結び付けたOperation権限によって制限されます。
...
- **ユーザーが承認する契約**: 目的、成果、調査境界、対象・Action・Effectを結び付けた権限、自動追加上限、検証水準
...
**Task Card** は、1回のWorker呼び出しに対する、Worker非依存の実行契約です。具体的な目的、許可された対象・Action・EffectのOperation、カード全体の上限、完了条件、中断条件を定義します。
...
- **短い承認画面**: ユーザーが確認するのは、目的、成果、調査境界、作用Operationまたは境界付き認可ルール、自動追加上限、検証、再承認条件だけ
- **Revision付き契約**: Task CardとWorker結果を、順序付きAuthorization Source Sequenceから導出した1つの有効な承認契約Revisionへ結び付ける
...
- **契約監査**: Workerが報告したOperation、対象、Action、Effect、上限、根拠、Revisionを確認してから進行する
...
実行時に、許可されたAgentから候補を選び、選択したWorkerの有効な定義を確認して、境界付きタスクとの意味的な互換性を判定します。Worker固有の入力キー、ラッパー、スキーマ、入力言語要件は採用しません。互換Workerは、DANDORI固有の入力規約を要求せず、自己完結した依頼を処理できる必要があります。Worker選択は実行品質には影響しますが、承認範囲を広げる根拠にはなりません。
...
Approved Contract revision
   ↓
Flow Ledger: 成果条件、認可Operationと対象、上限、重要な根拠
   ↓
...
**再承認**
目的、成果、境界、対象・Action・Effectの権限、自動追加上限、検証水準を広げる場合のみ
```
...
認可はOperation単位で行います。各権限は、調査境界または正確な作用対象／境界付き認可ルール、Action、そのActionが生み得るすべてのEffectを結び付けます。契約全体のAction／Effect一覧は上限と要約であり、それだけで対象とActionの組み合わせを認可しません。
...
候補を再承認なしで認可できるのは、正確な識別子、承認境界内であること、成果条件との対応、具体的な根拠位置、必要な対象・Action・EffectのOperation、リスク、自動追加上限を確認できる場合だけです。候補を新しい探索起点にはせず、発見した呼び出し内では作用させません。
...
有効な契約は、追記専用のAuthorization Source Sequenceを順番に適用して導出します。承認済みTFR／TFCは契約を拡張でき、ユーザーの明示的な縮小は追加承認なしで正規化された縮小差分として記録します。自由記述の原文は監査用であり、実行可能な権限の正本にはせず、削除した権限を暗黙に復元しません。
...
4. workspace、user、organization、extension、追加探索先にある同名定義の重複を削除または無効化する
